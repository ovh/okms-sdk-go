...
// Defines values for KeyFormatTypes.
const (
	JWK   KeyFormatTypes = "JWK"
	PKCS1 KeyFormatTypes = "PKCS1"
	PKCS8 KeyFormatTypes = "PKCS8"
	RAW   KeyFormatTypes = "RAW"
)

// Valid indicates whether the value is a known member of the KeyFormatTypes enum.
func (e KeyFormatTypes) Valid() bool {
	switch e {
	case JWK:
		return true
	case PKCS1:
...
// Defines values for ServiceKeyClassEnum.
const (
	KEYPAIR   ServiceKeyClassEnum = "KEY_PAIR"
	PUBLICKEY ServiceKeyClassEnum = "PUBLIC_KEY"
	SECRETKEY ServiceKeyClassEnum = "SECRET_KEY"
)

// Valid indicates whether the value is a known member of the ServiceKeyClassEnum enum.
func (e ServiceKeyClassEnum) Valid() bool {
	switch e {
	case KEYPAIR:
		return true
	case PUBLICKEY:
...
type CreateImportServiceKeyRequest struct {
	...
	// Extractable Whether the key material can be exported (plain or wrapped). Default: false; public-only keys default true.
	Extractable *bool `json:"extractable,omitempty"`

	// Id Optional user-defined key id (UUID)
	...
	// WrappedKeys Wrapped key material to unwrap and import. When present, triggers wrapped-import mode: type, size, and curve are inferred from the decrypted material.
...
type GetConfigResponse struct {
	...
	// RequestId Example: a1a5477a-a4a2-42bb-bcc7-cf8428878d0b
	RequestId *string `json:"request_id,omitempty"`
}
...
type GetMetadataResponse struct {
	...
	// RequestId Example: 79b66eba-273e-4f7a-b954-78ed7e70b85d
	RequestId *string `json:"request_id,omitempty"`
}
...
type GetSecretResponse struct {
	...
	// RequestId Example: 7298c8ee-46ad-44a9-ae4b-6266d55132a3
	RequestId *string `json:"request_id,omitempty"`
}
...
type GetServiceKeyResponse struct {
	...
	// Class Kind of service key material: SECRET_KEY (symmetric), KEY_PAIR (asymmetric with private and public parts), or PUBLIC_KEY (asymmetric, public part only).
	Class *ServiceKeyClassEnum `json:"class,omitempty"`
	Curve *Curves              `json:"curve,omitempty"`

	// Id Id of the key, UUID
	Id openapi_types.UUID `json:"id"`

	// Keys Json Web Key Set representing keys in request (plain export)
	...
	// WrappedKeys Wrapped key material (wrapped-export mode only). Present instead of `keys` on wrap mode.
	WrappedKeys *[]WrappedKeyEntry `json:"wrappedKeys,omitempty"`
}
...
// KeyFormatTypes Format of the plaintext key material used for wrap/unwrap.
type KeyFormatTypes string
...
type PatchServiceKeyRequest struct {
	// Extractable Whether the key material can be exported. Making a key extractable permanently sets never_extractable to false.
	Extractable *bool `json:"extractable,omitempty"`
...
type PostSecretRequest struct {
	...
	//
	// Example: {"baz":["abc","123"],"foo":"bar","zip":{"zap":"456"}}
...
type SecretDataSubkeys struct {
	...
	//
	// Example: {"bar":{"baz":null},"foo":null,"quux":null}
...
type SecretMetadata struct {
	...
	//
	// Example: {"1":{"created_time":"2025-08-22T02:24:06.945319214Z","deletion_time":"","destroyed":false},"2":{"created_time":"2025-08-04T06:24:06.945319214Z","deletion_time":"","destroyed":false},"3":{"created_time":"2025-09-22T09:36:43.986212308Z","deletion_time":"","destroyed":false}}
...
// SecretV2CustomMetadata Example: {"bar":"123","foo":"abc"}
...
type SecretV2Metadata struct {
	...
	// CustomMetadata Example: {"bar":"123","foo":"abc"}
...
type SecretVersionMetadata struct {
	...
	// CustomMetadata Custom metadata attached to the secret
	//
	// Example: {"owner":"jdoe","ref":"7cedc814-645b-4803-8e03-2d6f7c41a3df"}
	CustomMetadata *map[string]string `json:"custom_metadata,omitempty"`
...
// ServiceKeyClassEnum Kind of service key material: SECRET_KEY (symmetric), KEY_PAIR (asymmetric with private and public parts), or PUBLIC_KEY (asymmetric, public part only).
type ServiceKeyClassEnum string
...
// WrappedKeyEntry Single wrapped key material entry used in wrapped export and import
type WrappedKeyEntry struct {
	// Ciphertext Encrypted key material in format JWE Compact Serialization.
	Ciphertext string `json:"ciphertext"`

	// KeyFormatType Format of the plaintext key material used for wrap/unwrap.
	KeyFormatType KeyFormatTypes `json:"keyFormatType"`

	// WrappingKeyId ID (UUID) of the transport key used to wrap/unwrap. Validated as a UUID by the server.
	WrappingKeyId openapi_types.UUID `json:"wrappingKeyId"`
}
...
type GetServiceKeyParams struct {
	// Format Formatting options for key representation. Ignored in wrapped-export mode.
	Format *KeyFormats `form:"format,omitempty" json:"format,omitempty"`

	// WrappingKeyId ID of the transport (wrapping) key. When present, triggers wrapped-export mode: key material is encrypted with this key and returned in `wrappedKeys`. Must reference a SOFTWARE key in the same domain that has the `wrapKey` usage.
	WrappingKeyId *openapi_types.UUID `form:"wrappingKeyId,omitempty" json:"wrappingKeyId,omitempty"`

	// WrappedKeyFormat Format of the plaintext key material before wrapping. Required when `wrappingKeyId` is present.
	WrappedKeyFormat *KeyFormatTypes `form:"wrappedKeyFormat,omitempty" json:"wrappedKeyFormat,omitempty"`

	// WrappingAlgorithm Wrapping algorithm to use. Must be compatible with the `wrappingKeyId`. Required when `wrappingKeyId` is present.
	WrappingAlgorithm *WrappingAlgorithms `form:"wrappingAlgorithm,omitempty" json:"wrappingAlgorithm,omitempty"`
}
