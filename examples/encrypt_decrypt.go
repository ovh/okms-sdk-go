...
func encryptDecrypt(ctx context.Context, okmsClient *okms.Client, okmsId uuid.UUID) {
	...
	respAes, err := okmsClient.GenerateSymmetricKey(ctx, okmsId, types.N256, "AES key example", types.SOFTWARE, "", []types.CryptographicUsages{types.Encrypt, types.Decrypt})
	if err != nil {
		panic(err)
	}
