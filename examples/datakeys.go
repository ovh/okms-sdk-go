...
func dataKeyEncryptDecrypt(ctx context.Context, okmsClient *okms.Client, okmsId uuid.UUID) {
	...
	respAes, err := okmsClient.GenerateSymmetricKey(ctx, okmsId, types.N256, "AES key example", types.SOFTWARE, "", []types.CryptographicUsages{types.WrapKey, types.UnwrapKey})
	if err != nil {
		panic(err)
	}
