...
func signVerify(ctx context.Context, kmsClient *okms.Client, okmsId uuid.UUID) {
	...
	respECDSA, err := kmsClient.GenerateECKeyPair(ctx, okmsId, types.P256, "ECDSA key-pair example", types.SOFTWARE, "", []types.CryptographicUsages{types.Sign, types.Verify})
	if err != nil {
		panic(err)
	}
