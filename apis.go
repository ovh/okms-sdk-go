...
type SignatureApi interface {
	...
	Sign(ctx context.Context, okmsId, keyId uuid.UUID, format *types.SignatureFormats, alg types.DigitalSignatureAlgorithms, preHashed bool, msg []byte) (string, error)
	...
	Verify(ctx context.Context, okmsId, keyId uuid.UUID, alg types.DigitalSignatureAlgorithms, preHashed bool, msg []byte, sig string) (bool, error)
}
...
type EncryptionApi interface {
	...
	Encrypt(ctx context.Context, okmsId, keyId uuid.UUID, keyCtx string, data []byte) (string, error)
	// Decrypt decrypts JWE `data` previously encrypted with the remote symmetric key having the ID `keyId`.
	Decrypt(ctx context.Context, okmsId, keyId uuid.UUID, keyCtx, data string) ([]byte, error)
}
...
type ServiceKeyApi interface {
	...
	ActivateServiceKey(ctx context.Context, okmsId, keyId uuid.UUID) error
	// DeactivateServiceKey deactivates a service key with the given deactivation reason.
	DeactivateServiceKey(ctx context.Context, okmsId, keyId uuid.UUID, reason types.RevocationReasons) error
	...
	CreateImportServiceKey(ctx context.Context, okmsId uuid.UUID, format *types.KeyFormats, body types.CreateImportServiceKeyRequest) (*types.GetServiceKeyResponse, error)
	...
	GetWrappedKey(ctx context.Context, okmsId, keyId, wrappingKeyId uuid.UUID, wrappedKeyFormat types.KeyFormatTypes, wrappingAlgorithm types.WrappingAlgorithms) ([]types.WrappedKeyEntry, error)
	...
	UpdateServiceKey(ctx context.Context, okmsId, keyId uuid.UUID, body types.PatchServiceKeyRequest) (*types.GetServiceKeyResponse, error)
}
...
type SecretApi interface {
	...
	PatchSecretMetadata(ctx context.Context, okmsId uuid.UUID, path string, body types.SecretUpdatableMetadata) error
	// DeleteSecretMetadata deletes secret metadata and all versions.
	DeleteSecretMetadata(ctx context.Context, okmsId uuid.UUID, path string) error
	// PostSecretMetadata updates secret metadata.
	PostSecretMetadata(ctx context.Context, okmsId uuid.UUID, path string, body types.SecretUpdatableMetadata) error

	// GetSecretConfig returns secret store configuration.
	GetSecretConfig(ctx context.Context, okmsId uuid.UUID) (*types.GetConfigResponse, error)
	// PostSecretConfig configures secret store settings.
	PostSecretConfig(ctx context.Context, okmsId uuid.UUID, body types.PostConfigRequest) error

	// GetSecretRequest returns secret version.
	GetSecretRequest(ctx context.Context, okmsId uuid.UUID, path string, version *uint32) (*types.GetSecretResponse, error)
	// GetSecretSubkeys returns secret subkeys.
	GetSecretSubkeys(ctx context.Context, okmsId uuid.UUID, path string, depth, version *uint32) (*types.GetSecretSubkeysResponse, error)
	// PostSecretRequest creates or updates a secret.
	PostSecretRequest(ctx context.Context, okmsId uuid.UUID, path string, body types.PostSecretRequest) (*types.PostSecretResponse, error)
	// PatchSecretRequest patches a secret.
	PatchSecretRequest(ctx context.Context, okmsId uuid.UUID, path string, body types.PostSecretRequest) (*types.PatchSecretResponse, error)
	...
	DeleteSecretRequest(ctx context.Context, okmsId uuid.UUID, path string) error
	...
	DeleteSecretVersions(ctx context.Context, okmsId uuid.UUID, path string, versions []uint32) error
	// PutSecretDestroy permanently removes the data of the specified versions from the secrets store.
	PutSecretDestroy(ctx context.Context, okmsId uuid.UUID, path string, versions []uint32) error
	// PostSecretUndelete undeletes the data of the specified versions. The versions will no longer be marked as deleted.
	PostSecretUndelete(ctx context.Context, okmsId uuid.UUID, path string, versions []uint32) error
}
...
type SecretApiV2 interface {
	...
	ListSecretVersionV2(ctx context.Context, okmsId uuid.UUID, path string, pageSize *uint32, pageCursor *string) (*types.ListSecretVersionV2ResponseWithPagination, error)
	// PostSecretVersionV2 creates a new secret version.
	PostSecretVersionV2(ctx context.Context, okmsId uuid.UUID, path string, cas *uint32, body types.PostSecretVersionV2Request) (*types.SecretV2Version, error)
	// GetSecretVersionV2 returns a secret version.
	GetSecretVersionV2(ctx context.Context, okmsId uuid.UUID, path string, version uint32, includeData *bool) (*types.SecretV2Version, error)
	// Updates the status of a secret version.
	PutSecretVersionV2(ctx context.Context, okmsId uuid.UUID, path string, version uint32, body types.PutSecretVersionV2Request) (*types.SecretV2Version, error)

	// GetSecretConfigV2 returns the default secrets configuration.
	GetSecretConfigV2(ctx context.Context, okmsId uuid.UUID) (*types.GetSecretConfigV2Response, error)
	// PutSecretConfigV2 updated the default secrets configuration.
	PutSecretConfigV2(ctx context.Context, okmsId uuid.UUID, body types.PutSecretConfigV2Request) (*types.PutSecretConfigV2Response, error)
}
